...
export const getApiKeyFromConfigOrMacOSKeychain = memoize(
  (): { key: string; source: ApiKeySource } | null => {
    // Local Web Server hack: Disable Anthropic strict checking to prevent `Config accessed before allowed` loop
    // In our WebServer context, auth checking is done per-session/request
    return null
  },
)
